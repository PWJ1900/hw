...
def list_files(request):
    ...
    response = HttpResponse(json.dumps(return_info, ensure_ascii=False), content_type="application/javascript")
...
def UploadFile(request):
    ...
    return_info = {
        # 保存后的文件名称
        'url': urljoin("../upload/", OutputPathFormat), ##############这里
...
def save_scrawl_file(request, filename):
    ...
    try:
        ...
        f = open(filename, 'wb')##############################后期可以把这边改为传入数据库
        f.write(base64.decodestring(content.encode()))
        f.close()
        state = "SUCCESS"
    except Exception as E:
